    ...
    <div className={`flex flex-col min-h-screen animate-fade-in relative z-20 pb-10 transition-colors`}>
       ...
       <div className="flex-1 px-5 flex flex-col gap-6 relative z-10">
          ...
          <div className="flex items-center gap-4">
             <div className="relative cursor-pointer" onClick={() => { setEditForm(profile); setShowEditProfile(true); }}>
                ...
                <div className="absolute bottom-0 right-0 w-6 h-6 bg-[#F5A623] text-black rounded-full flex items-center justify-center border-2 border-[#0a0a0a]">
                  <Edit3 size={12} />
                </div>
             </div>
             <div>
          ...
          {/* Stats Row */}
          <div className={`card ${settings.darkMode ? 'border-[#333] divide-[#333] bg-[#141414]' : 'bg-white border-gray-200 divide-gray-200'} grid grid-cols-3 divide-x p-0 shadow-sm`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
               <h3 className="text-sm font-bold tracking-wider">My FSSAI Reports</h3>
               <button onClick={() => navigate('/reports')} className="text-[10px] uppercase font-bold text-[#F5A623] tracking-widest">View All</button>
            </div>
            
            <div onClick={() => navigate('/reports')} className={`card p-4 ${settings.darkMode ? 'border-[#333] bg-[#141414]' : 'bg-white border-gray-200'} flex items-center justify-between hover:border-[#F5A623]/50 cursor-pointer transition-colors shadow-sm`}>
               <div className="flex items-center gap-3">
                  ...
                  <div>
                     <p className="font-bold text-sm">Violation Reports</p>
                     <p className="text-xs text-gray-400">{reportsCount} notices pending review</p>
                  </div>
               </div>
          ...
          {/* Account Settings */}
          <div>
             <h3 className="text-sm font-bold tracking-wider mb-3">Settings</h3>
             <div className={`card ${settings.darkMode ? 'border-[#333] bg-[#141414] divide-[#1c1c1c]' : 'bg-white border-gray-200 divide-gray-100'} p-0 flex flex-col divide-y shadow-sm`}>
                ...
                {/* External Link */}
                <div onClick={() => window.open('https://fssai.gov.in', '_blank')} className="flex items-center justify-between p-4 cursor-pointer hover:bg-black/5 transition-colors">
                   <div className="flex items-center gap-3">
                      <FileText size={18} className="text-[#F5A623]" />
                      <span className="text-sm font-medium">FSSAI Guidelines</span>
                   </div>
       ...
       {showEditProfile && (
         <div className="fixed inset-0 bg-black/60 z-[100] flex items-end animate-fade-in backdrop-blur-sm">
            <div className={`w-full ${settings.darkMode ? 'bg-[#0a0a0a] border-[#333]' : 'bg-white border-gray-200'} border-t rounded-t-[2.5rem] p-6 pb-safe animate-slide-up`}>
               <div className="flex justify-between items-center mb-6">
                 <h2 className="text-xl font-bold">Edit Profile</h2>
                 <button onClick={() => setShowEditProfile(false)} className="p-2 bg-gray-500/10 rounded-full text-gray-400">
                   <X size={20} />
                 </button>
               </div>
               ...
               <form onSubmit={saveProfile} className="flex flex-col gap-4">
                 <div>
                   <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest pl-2 mb-1 block">Full Name</label>
                   <input required value={editForm.name} onChange={e=>setEditForm({...editForm, name: e.target.value})} className={`w-full ${settings.darkMode ? 'bg-[#1c1c1c] border-[#333] text-white' : 'bg-gray-50 border-gray-200 text-black'} border focus:border-[#F5A623] rounded-2xl py-3.5 px-4 outline-none text-sm`} />
                 </div>
                 <div>
                   <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest pl-2 mb-1 block">Email</label>
                   <input required type="email" value={editForm.email} onChange={e=>setEditForm({...editForm, email: e.target.value})} className={`w-full ${settings.darkMode ? 'bg-[#1c1c1c] border-[#333] text-white' : 'bg-gray-50 border-gray-200 text-black'} border focus:border-[#F5A623] rounded-2xl py-3.5 px-4 outline-none text-sm`} />
                 </div>
                 <div>
                   <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest pl-2 mb-1 block">Phone Number</label>
                   <input required value={editForm.phone} onChange={e=>setEditForm({...editForm, phone: e.target.value})} className={`w-full ${settings.darkMode ? 'bg-[#1c1c1c] border-[#333] text-white' : 'bg-gray-50 border-gray-200 text-black'} border focus:border-[#F5A623] rounded-2xl py-3.5 px-4 outline-none text-sm`} />
                 </div>
                 <div className="mb-2">
                   <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest pl-2 mb-1 block">Badge ID</label>
                   <input required value={editForm.badgeId} onChange={e=>setEditForm({...editForm, badgeId: e.target.value})} className={`w-full ${settings.darkMode ? 'bg-[#1c1c1c] border-[#333] text-white' : 'bg-gray-50 border-gray-200 text-black'} border focus:border-[#F5A623] rounded-2xl py-3.5 px-4 outline-none text-sm`} />
                 </div>
       ...
       {showConnection && (
         <div className="fixed inset-0 bg-black/60 z-[100] flex items-end animate-fade-in backdrop-blur-sm" onClick={() => setShowConnection(false)}>
            <div className={`w-full ${settings.darkMode ? 'bg-[#0a0a0a] border-[#333]' : 'bg-white border-gray-200'} border-t rounded-t-[2.5rem] p-6 pb-safe animate-slide-up`} onClick={e=>e.stopPropagation()}>
               <h2 className="text-xl font-bold mb-6">Default Scanner Link</h2>
               <div className="flex flex-col gap-2 mb-6">
                 {['Bluetooth', 'Wi-Fi', 'USB OpenSerial'].map(conn => (
                   <button key={conn} onClick={() => { updateSetting('connectionMethod', conn); setShowConnection(false); }} className={`p-4 rounded-2xl font-bold flex justify-between items-center ${settings.connectionMethod === conn ? 'bg-[#F5A623]/10 text-[#F5A623] border border-[#F5A623]/30' : settings.darkMode ? 'bg-[#1c1c1c] text-white border border-[#333]' : 'bg-gray-50 text-black border border-gray-200'}`}>
                     {conn}
       ...
       {showLogout && (
         <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-5 animate-fade-in backdrop-blur-md" onClick={() => setShowLogout(false)}>
            <div className={`w-full max-w-sm ${settings.darkMode ? 'bg-[#141414] border-[#333]' : 'bg-white border-gray-200'} p-6 rounded-3xl border shadow-2xl animate-slide-up text-center`} onClick={e=>e.stopPropagation()}>
               ...
               <h2 className="text-xl font-black mb-2">Sign Out?</h2>
               <p className="text-gray-400 text-sm mb-6">You will need to re-authenticate with your FSSAI credentials to access the scanner.</p>
               
               <div className="flex gap-3">
                 <button onClick={() => setShowLogout(false)} className={`flex-1 py-3 rounded-xl font-bold ${settings.darkMode ? 'bg-[#1c1c1c] text-white' : 'bg-gray-100 text-black'}`}>
                   Cancel
                 </button>

...
export const RecommendationResults = ({
  ...
  return (
    <div className="space-y-8">
      ...
      {/* AI Deep Analysis Section (if present) */}
      {aiResult && (
        <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 rounded-2xl p-6 md:p-8 text-white shadow-md border border-emerald-500/30">
          <div className="flex items-center gap-2 mb-4">
            <div className="p-2 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-400/30">
              <Sparkles className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-lg font-bold font-serif text-emerald-100">
                Gemini AI Clinical Lipid Analysis
              </h3>
              <p className="text-xs text-emerald-300/80">AI-generated customized medical report based on your combined conditions</p>
            </div>
          </div>

          <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-xs text-slate-200 leading-relaxed mb-6">
            <p className="font-medium text-emerald-200 mb-1">Doctor Summary & Rationale:</p>
            {aiResult.summary}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-1.5">
                ...
                <span>AI Prescribed Oils to Consume</span>
              </h4>
              <div className="space-y-2.5">
                {aiResult.recommendedOils.map((item, idx) => (
                  <div key={idx} className="p-3.5 rounded-xl bg-white/10 border border-white/10 space-y-1.5 text-xs">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-white font-serif text-sm">{item.name}</span>
                      <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 font-mono text-[10px] font-semibold border border-emerald-400/30">
                        {item.recommendedQuantity}
                      </span>
                    </div>
                    <p className="text-slate-300 text-[11px]">{item.primaryReason}</p>
                    <div className="flex flex-wrap gap-1 pt-1">
                      {item.bestCookingMethods.map((m, i) => (
            ...
            {/* AI Avoid & Clinical Tips */}
            <div className="space-y-3">
              <h4 className="text-xs font-bold uppercase tracking-wider text-rose-400 flex items-center gap-1.5">
                <XCircle className="w-4 h-4 text-rose-400" />
                <span>AI Prescribed Oils to Avoid</span>
              </h4>
              <div className="space-y-2.5">
                {aiResult.oilsToAvoid.map((item, idx) => (
                  <div key={idx} className="p-3.5 rounded-xl bg-rose-950/40 border border-rose-500/30 space-y-1 text-xs">
                    <span className="font-bold text-rose-200 font-serif text-sm block">{item.name}</span>
                    <p className="text-rose-300/90 text-[11px]">{item.healthRisk}</p>
                    <p className="text-[10px] text-emerald-300 font-medium pt-1">
                      ✨ Healthy Swap: {item.alternativeSwap}
                    </p>
                  </div>
                ))}
              </div>
              ...
              {/* Clinical Advice */}
              {aiResult.clinicalAdvice && aiResult.clinicalAdvice.length > 0 && (
                <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/20 space-y-1.5 text-xs mt-3">
                  <span className="font-bold text-amber-300 text-xs block">Key Dietary Tips:</span>
                  <ul className="list-disc list-inside space-y-1 text-amber-100/90 text-[11px]">
                    {aiResult.clinicalAdvice.map((tip, i) => (
                      <li key={i}>{tip}</li>
                    ))}
                  </ul>
                </div>
